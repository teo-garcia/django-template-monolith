...
Django Ninja derives each operation's schema from its declared response model,
which describes the handler's return value. `ResponseEnvelopeMiddleware` then
wraps that value under `data`, so without this pass the spec would describe a
payload clients never actually receive.
"""
...
def apply_success_envelope(schema: dict[str, Any]) -> dict[str, Any]:
    """Rewrite every wrapped 2xx response to describe the envelope."""
